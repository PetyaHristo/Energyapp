...
$stmt = $pdo->prepare("SELECT day, energy FROM energy_data WHERE user_id = ? AND month = ?");
...
// Последна дата
$lastUpdateStmt = $pdo->prepare("SELECT MAX(day) as last_day FROM energy_data WHERE user_id = ? AND month = ?");
$lastUpdateStmt->execute([$user_id, $month]);
$lastDay = $lastUpdateStmt->fetchColumn();
...
<body>
    <div class="wrapper">
        ...
        <p class="last-update">
            📅 Последни данни до ден <?= $lastDay ?: 'няма данни' ?> за месец <?= date("F", mktime(0, 0, 0, $month, 1)) ?>
        </p>
        ...
        <table>
            ...
            <?php for ($day = 1; $day <= 31; $day++): ?>
                <tr>
                    <td><?= $day ?></td>
                    <td><?= isset($data[$day]) ? round($data[$day], 2) : '-' ?></td>
                </tr>
            <?php endfor; ?>
        </table>
...
// Запазване на ръчно въведени стойности
if (isset($_POST['save'])) {
...
<body>
    ...
    <script>
        ...
        // Автоматично презареждане на всеки 60 секунди
